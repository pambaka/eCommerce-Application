...
export default class AddressSectionComponent extends BaseComponent {
  ...
  private async handleSaveButtonClick() {
    ...
    const originalParent = this.node.parentNode;
    const originalNextSibling = this.node.nextSibling;
    if (originalParent) {
      if (originalNextSibling) {
        originalParent.insertBefore(this.node, originalNextSibling);
      } else {
        originalParent.appendChild(this.node);
      }
    }
    this.updateButtonVisibility();
  }
