...
//         const success = await customerUpdater.updateAddress(action, this.address.id || '', this.address);
//         if (success) {
//           this.onSaveButtonClick();
//         } else {
...
//       const customerUpdater = new CustomerUpdater();
//       const success = await customerUpdater.updateAddress('removeAddress', this.address.id || '');
//       if (success) {
//         this.node.remove();
//         this.onDeleteButtonClick();
//       }
  ...
  constructor(address: Address, index: number, userInfo: CustomerIncomeData, isNew: boolean = false) {
    ...
    this.saveButton = createSaveButton(async () => {
      if (this.areAllFieldsValid()) {
        ...
        if (addressIdOrKey) {
          Customer.logIn(this.userInfo.firstName);
          ...
          showModal('Address key or ID is missing!', '', false);
        }
      }
    });
